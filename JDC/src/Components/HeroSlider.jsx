...
export default function HeroSlider() {
  ...
  const uniqueModels = searchFilters.brand
    ? [...new Set(allVehicles.filter(v => v.brand === searchFilters.brand).map(v => v.model))].filter(Boolean).sort()
    : [];
  
  // Get unique categories
  const uniqueCategories = [...new Set(allVehicles.map(v => v.category))].filter(Boolean).sort();
    ...
    <>
      <div className="relative">
        <section className="relative h-[400px] sm:h-[450px] md:h-[500px] lg:h-[550px] flex items-start pt-4 sm:pt-8 md:pt-12 overflow-hidden">
          ...
          <button
            onClick={prevSlide}
            className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-30 p-2 sm:p-3 bg-white/10 backdrop-blur-sm hover:bg-white/20 text-white rounded-full transition-all duration-300 border border-white/30 hover:border-white/50"
          ...
          <button
            onClick={nextSlide}
            className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-30 p-2 sm:p-3 bg-white/10 backdrop-blur-sm hover:bg-white/20 text-white rounded-full transition-all duration-300 border border-white/30 hover:border-white/50"
                ...
                <div className="grid grid-cols-5 gap-4">
                  <div className="relative">
                    <select 
                      ...
                      className="w-full px-4 py-3 bg-gray-100 border-0 rounded-xl text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-red-600 focus:bg-white transition-all appearance-none cursor-pointer pr-10 text-sm"
                    >
                  ...
                  <div className="relative">
                    <select 
                      ...
                      disabled={!searchFilters.brand || uniqueModels.length === 0}
                      className="w-full px-4 py-3 bg-gray-100 border-0 rounded-xl text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-red-600 focus:bg-white transition-all appearance-none cursor-pointer pr-10 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transform hover:scale-105 text-sm"
                  >
        ...
        {/* Search Bar - Mobile and Tablet only - Full width breakout */}
              ...
              <div className="space-y-3">
                <div className="relative">
                  <select 
                    value={searchFilters.category}
                    onChange={(e) => setSearchFilters(prev => ({ ...prev, category: e.target.value }))}
                    className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600 transition-all appearance-none cursor-pointer pr-10 text-sm"
                  >
                    <option value="">Catégorie</option>
                    {uniqueCategories.map(cat => (
                      <option key={cat} value={cat}>{cat}</option>
                ...
                <div className="relative">
                  <select 
                    ...
                    onChange={(e) => setSearchFilters(prev => ({ ...prev, model: e.target.value }))}
                    disabled={!searchFilters.brand || uniqueModels.length === 0}
                    className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600 transition-all appearance-none cursor-pointer pr-10 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <option value="">Modèle</option>
                    {uniqueModels.map(model => (
                      <option key={model} value={model}>{model}</option>
                ...
                <button
                  onClick={handleSearch}
                  className="w-full px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transform hover:scale-105 text-sm"
                >
                  <Search className="w-4 h-4" />
                  Rechercher
                </button>
